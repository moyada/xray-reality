...
function system_check() {
  ...
  if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
    ...
    INS="yum install -y"
    ${INS} wget
    # wget -N -P /etc/yum.repos.d/ https://raw.githubusercontent.com/moyada/Xray_onekey/${github_branch}/basic/nginx.repo
...
function dependency_install() {
  ...
  if ! command -v jq; then
    wget -P /usr/bin https://raw.githubusercontent.com/moyada/xray-reality/${github_branch}/binary/jq && chmod +x /usr/bin/jq
    judge "安装 jq"
  fi
...
function domain_check() {
  read -rp "请输入你的域名信息(eg: www.moyada.com):" domain
...
function update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/moyada/xray-reality/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$(echo -e "$shell_version\n$ol_version" | sort -rV | head -1)" ]]; then
    print_ok "存在新版本，是否更新 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/moyada/xray-reality/${github_branch}/install.sh
      print_ok "更新完成"
...
function configure_nginx() {
  ...
  cd /etc/nginx/conf.d/ && rm -f ${domain}.conf && wget -O ${domain}.conf https://raw.githubusercontent.com/moyada/Xray_onekey/${github_branch}/config/web.conf
...
function xray_install() {
  ...
  curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh | bash -s -- install --version 25.7.26
  # curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh | bash -s -- install --beta
  # curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh | bash -s -- install --version 24.10.31
  judge "Xray 安装"
...
function configure_web() {
  ...
  case $webpage in
  [yY][eE][sS] | [yY])
    wget -O web.tar.gz https://raw.githubusercontent.com/moyada/Xray_onekey/main/basic/web.tar.gz
...
function vless_xtls-rprx-direct_link() {
  ...
  print_ok "vless://$UUID@$DOMAIN:$PORT?security=tls&flow=$FLOW#TLS_moyada-$DOMAIN"

  print_ok "URL 链接 (VLESS + TCP + XTLS)"
  print_ok "vless://$UUID@$DOMAIN:$PORT?security=xtls&flow=$FLOW#XTLS_moyada-$DOMAIN"
  print_ok "-------------------------------------------------"
  print_ok "URL 二维码 (VLESS + TCP + TLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?security=tls%26flow=$FLOW%23TLS_moyada-$DOMAIN"

  print_ok "URL 二维码 (VLESS + TCP + XTLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?security=xtls%26flow=$FLOW%23XTLS_moyada-$DOMAIN"
}
...
function ws_link() {
  ...
  print_ok "URL 链接 (VLESS + TCP + TLS)"
  print_ok "vless://$UUID@$DOMAIN:$PORT?security=tls#TLS_moyada-$DOMAIN"

  print_ok "URL 链接 (VLESS + TCP + XTLS)"
  print_ok "vless://$UUID@$DOMAIN:$PORT?security=xtls&flow=$FLOW#XTLS_moyada-$DOMAIN"

  print_ok "URL 链接 (VLESS + WebSocket + TLS)"
  print_ok "vless://$UUID@$DOMAIN:$PORT?type=ws&security=tls&path=%2f${WS_PATH_WITHOUT_SLASH}%2f#WS_TLS_moyada-$DOMAIN"
  print_ok "-------------------------------------------------"
  print_ok "URL 二维码 (VLESS + TCP + TLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?security=tls%23TLS_moyada-$DOMAIN"

  print_ok "URL 二维码 (VLESS + TCP + XTLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?security=xtls%26flow=$FLOW%23XTLS_moyada-$DOMAIN"

  print_ok "URL 二维码 (VLESS + WebSocket + TLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?type=ws%26security=tls%26path=%2f${WS_PATH_WITHOUT_SLASH}%2f%23WS_TLS_moyada-$DOMAIN"
}

function basic_information() {
  print_ok "VLESS+TCP+TLS+REALITY 安装成功"
...
menu() {
  ...
  echo -e "\t---authored by moyadag---"
  echo -e "\thttps://github.com/moyada\n"

  echo -e "当前已安装版本：${shell_mode}"
  echo -e "—————————————— 安装向导 ——————————————"""
  echo -e "${Green}0.${Font}  升级 脚本"
  echo -e "${Green}1.${Font}  安装 Xray (VLESS + TCP + XTLS + REALITY + H2)"
  echo -e "—————————————— 配置变更 ——————————————"
